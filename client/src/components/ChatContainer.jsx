...
const ChatContainer = () => {
  ...
  // ✅ Persist selectedUser in localStorage so refresh keeps the chat open
  useEffect(() => {
    if (selectedUser) {
 ...
 const handleSendMessage = async (e) => {
  ...
  await sendMessage({ text: input.trim(), receiver: selectedUser._id }); // <-- add receiver
  setInput('');
};
...
const handleSendImage = async (e) => {
  ...
  reader.onloadend = async () => {
    await sendMessage({ image: reader.result, receiver: selectedUser._id }); // <-- add receiver
    e.target.value = '';
  };
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        <img
          onClick={() => {
            ...
            localStorage.removeItem('selectedUser'); // ✅ clear on exit
          }}
        ...
        {messages.map((msg, index) => (
          ...
          >
            {msg.image ? (
              ...
              <p
                className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg break-all text-white mb-8 ${
